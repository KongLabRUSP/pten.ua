...
t3 <- data.table(dt3[, 1:4], 
                 dt3[, seq(6, 12, 2), with = FALSE]/ 
                   dt3[, seq(5, 11, 2), with = FALSE])
names(t3)[5:8] <- sName
t3
...
sum(ndx)
# Excluded 33,834 regions for now
tmp <- t(tmp[!ndx, ])
dim(tmp)
colnames(tmp) <- t3$start[!ndx]
...
p1 <- ggplot(data = df.v[var.keep.ndx,], 
  ...
  #                                "AF (DX1-9)")) +
  ggtitle("Biplot of 10,000 Random MIDAS Records") +
